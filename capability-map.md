...
## 〇·二、大神方案对照（repo-port 决策记录，2026-08-08）

对 archival-ai 各实现逻辑的 GitHub 成熟方案检索结果（置信度 High：L1-L6 完成）：
...
| **dateutil / dateparser** | 自然语言日期解析 | IGNORE | YYMMDD/范围格式自研正则已最优（通用库面向"yesterday"类自然语言） |

**结论**：archival-ai 的安全网/清洗/日期/冲突实现逻辑已被大神方案独立验证（f2 与 rnr 覆盖相同理念）；唯一可提取提升（NFC 归一化）已落地。**不是"没有方案"，而是"方案已覆盖 + 哲学边界明确"**——继续自研合理（查过、证明过）。
